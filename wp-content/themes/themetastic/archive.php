<?php
	...
	/*if ( function_exists( 'get_option_tree') ) {
		$themetastic_archivelayout = get_option_tree( 'value_archivesidebar' );
		$themetastic_bloglayout = get_option_tree( 'value_blogoverviewpostlayout' );
		$themetastic_blogpostino = get_option_tree('value_blogoverviewpostinfo', '', false, true );
		$nopostinfo = "";
		$portfolioheightlock = get_option_tree( 'value_portfoliolock' );
		$portfoliocategorysidebar = get_option_tree( 'value_portfolioarchivesidebar' );
		if(!is_array($themetastic_blogpostino)){ $themetastic_blogpostino = array(); $nopostinfo ="style='display:none;'"; }
	}*/
	
	$portfolioheightlock = $themeoptions['themetastic_portfoliolock'];
	$portfoliocategorysidebar = $themeoptions['themetastic_portfolioarchivesidebar'];
	...
	if(is_category() || is_archive()){
		...
		elseif(is_archive()){
			...
			if(is_tax()){
				...
				$posts_per_page = empty($themeoptions['themetastic_portfolioarchivenumber']) ? get_option('posts_per_page') : $themeoptions['themetastic_portfolioarchivenumber'];
			}
			else{
...
<div class="row">
    ...
    if(!is_tax()){?>
						...
						<div class="holderwrap">
							...
							<div class="foliotextholder">
								<div class="foliotextwrapper">
									...
									<?php if($themetastic_item_categories=="On" && !is_wp_error($categorylinks)){ ?><span class="itemcategories"><?php echo $categorylinks ?></span><?php } ?>
								</div>
							</div>
						</div>
					</div>
       ...
       <?php 
        if ($themetastic_activate_sidebar=="on" && is_tax()) { $sbmod = "style='margin-top: 0px !important;'"; } else { $sbmod = ""; }
        ?>
