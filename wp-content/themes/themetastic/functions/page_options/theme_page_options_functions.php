...
function show_custom_page_woocommerce_meta_box(){
	global $custom_page_woocommerce_meta_fields,$custom_meta_fields;
	$custom_meta_fields=$custom_page_woocommerce_meta_fields;
	show_custom_meta_box();
}
	...
	foreach ($custom_meta_fields as $field) {
		...
		echo '<tr class="'.$field['class'].'">
				...
				switch($field['type']) {
					...
					// text
					case 'text_side':
						echo '<td><input type="text" name="'.$field['id'].'" id="'.$field['id'].'" value="'.$meta.'" size="30" /><br><span class="description">'.$field['desc'].'</span></td><td></td>';
					break;
					...
					// image
					case 'image':
						$image_def = get_template_directory_uri().'/img/tiles/more.png';	
							...
							foreach ($tp_home_teasers as $teaser) {
								...
								if(in_array($teaser, $tp_showbiz_teasers) || $teaser_headline_short ==""){
									...
									if(strlen($teaser_headline_short)>14)
										...
										$teaser_list_used .= '<li class="widget ui-draggable"><input name="themetastic_home_teasers[]" type="hidden" value="'.$teaser.'">'.$teaser_headline_short.'</li>
									';
								}
							}
						}
						...
						if(is_array($tp_showbiz_teasers)){
							foreach ($tp_showbiz_teasers as $teaser) {
								...
								if(!is_array($tp_home_teasers) || !in_array($teaser, $tp_home_teasers))
									$teaser_list_unused .= '<li class="widget ui-draggable"><input name="themetastic_home_teasers[]" type="hidden" value="'.$teaser.'" disabled>'.$teaser_headline_short.'</li>
								';
							}
						}

						if (!strpos($teaser_list_used, 'value="themetastic_home_content">Page Content</li>'))
						$teaser_list_unused .= '<li class="widget ui-draggable"><input name="themetastic_home_teasers[]" type="hidden" value="themetastic_home_content" disabled>Page Content</li>';	
...
function save_custom_meta($post_id) {
    ...
    if(isset($_POST['post_type'])){
	    ...
	    if ('page' == $_POST['post_type']) {
			$custom_meta_fields = array_merge($custom_page_meta_fields,$custom_page_portfolio_meta_fields,$custom_page_woocommerce_meta_fields);
		}
